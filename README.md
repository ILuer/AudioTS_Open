...
- **两层安全**：站内安全由应用代码负责（严格 CSP 约束浏览器脚本）；站外安全（机器人防护、访问分析）由 Cloudflare 在边缘提供。
- **控制台提示属正常现象**：因严格 CSP 与 Cloudflare 边缘注入脚本冲突，浏览器可能出现 CSP 相关报错（如内联脚本被拦截）。这些提示不影响应用功能，也不削弱任何安全能力，可安全忽略。
