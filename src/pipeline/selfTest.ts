 ...
 *
 * 多模型可插拔改造（dev 分支）:
 *   session 名与张量名经 S()/io() 从「当前生效能力契约」解析，不再内联字面量。
 *   注意 H / V / N_GROUPS / DEC_FRAMES 是「内建架构的参考形状」——
 *   它们是本测试的期望值（断言用），不是模型参数，故不随契约变化。
 */
...
import type { SessionRole } from '@/types/capability';

/** 契约解析：session 名（运行期读取，manifest 变更后立即生效） */
const S = (role: SessionRole): string => sessionNameOf(getActiveCapability(), role);
/** 契约解析：张量名 */
const io = () => getActiveCapability().io;
...
export async function runSelfTest(
  ...
  try {
    ...
    const emb = new Float32Array(res.data![io().textEmbeds] ?? res.data![Object.keys(res.data!)[0]]);
  ...
  try {
    ...
    const emb = new Float32Array(res.data![io().codecEmbeds] ?? res.data![Object.keys(res.data!)[0]]);
  ...
  try {
    ...
    const res = await sm.runInference<{ success: boolean; data: Record<string, ArrayBuffer> }>(
      S('talker'),
      { [io().inputsEmbeds]: smallIn, [io().positionIds]: pos, [io().attentionMask]: mask },
      { [io().inputsEmbeds]: [1, 3, H], [io().positionIds]: [3, 1, 3], [io().attentionMask]: [1, 3] },
      'selftest',
    );
    const keys = Object.keys(res.data!);
    const logits = new Float32Array(res.data![io().logits] ?? res.data![keys[0]]);
    const hidden = new Float32Array(res.data![io().hiddenStates] ?? res.data![keys[1]]);
  ...
  try {
    ...
    const res = await sm.runInference<{ success: boolean; data: Record<string, ArrayBuffer> }>(
      S('codePredictor'),
      { [io().talkerHidden]: th, [io().codecIds]: cIds },
      { [io().talkerHidden]: [1, H], [io().codecIds]: [1, N_GROUPS] },
      'selftest',
    );
    const gl = new Float32Array(res.data![io().groupLogits] ?? res.data![Object.keys(res.data!)[0]]);
  ...
  try {
    ...
    const res = await sm.runInference<{ success: boolean; data: Record<string, ArrayBuffer> }>(
      S('residualEmbed'),
      { [io().codecIds]: cIds },
      { [io().codecIds]: [1, N_GROUPS] },
      'selftest',
    );
    const se = new Float32Array(res.data![io().stepEmbed] ?? res.data![Object.keys(res.data!)[0]]);
    const expectedLen = H;
  ...
  try {
    ...
    const res = await sm.runInference<{ success: boolean; data: Record<string, ArrayBuffer> }>(
      S('tokDecoder'),
      { [io().audioCodes]: codes },
      { [io().audioCodes]: [1, DEC_FRAMES, N_GROUPS] },
      'selftest',
    );
    const wav = new Float32Array(res.data![io().waveform] ?? res.data![Object.keys(res.data!)[0]]);
  ...
  try {
    ...
    const bodyEmb = await new Promise<Float32Array>(async (resolve) => {
      const r = await sm.runInference<{ success: boolean; data: Record<string, ArrayBuffer> }>(
        S('textEmbed'), { [io().textIds]: bodyIds }, { [io().textIds]: [1, 1] }, 'selftest');
      resolve(new Float32Array(r.data![io().textEmbeds] ?? r.data![Object.keys(r.data!)[0]]));
    });
    ...
    const specR = await sm.runInference<{ success: boolean; data: Record<string, ArrayBuffer> }>(
      S('textEmbed'), { [io().textIds]: specIds }, { [io().textIds]: [1, 3] }, 'selftest');
    const spec = new Float32Array(specR.data![io().textEmbeds] ?? specR.data![Object.keys(specR.data!)[0]]);
    ...
    const tRes = await sm.runInference<{ success: boolean; data: Record<string, ArrayBuffer> }>(
      S('talker'), { [io().inputsEmbeds]: testTalkerIn, [io().positionIds]: tpos, [io().attentionMask]: tmask },
      { [io().inputsEmbeds]: [1, T, H], [io().positionIds]: [3, 1, T], [io().attentionMask]: [1, T] }, 'selftest');
    const tk = Object.keys(tRes.data!);
    const tlogits = new Float32Array(tRes.data![io().logits] ?? tRes.data![tk[0]]);
...
export async function decodeReferenceCodes(sm: OrtSessionManager): Promise<Float32Array> {
  ...
  for (let s = 0; s < totalFrames; s += DEC_FRAMES) {
    ...
    const res = await sm.runInference<{success:boolean;data:Record<string,ArrayBuffer>}>(
      S('tokDecoder'),
      { [io().audioCodes]: chunk },
      { [io().audioCodes]: [1, DEC_FRAMES, 16] },
      'decode_ref',
    );
    const wav = new Float32Array(res.data![io().waveform] ?? Object.values(res.data!)[0]);
